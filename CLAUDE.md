...
## 表示テーマ: ライト/セピア/ダーク(2026-08-23)

計画書§5にあった「テーマ切り替え」を実装。既定(初回起動・未設定時)は**セピア**のままにし、
既存ユーザーの見た目を変えないようにした(これまでの`--bg:#f5f1e8`等の配色がそのままセピア扱いになる)。

- **`--accent`と`--accent-text`を分けた**のが設計の要。`--accent`は「白文字と組み合わせる塗り」
  (ヘッダー・ボトムバー・ボタン)用で常に白文字が読めるだけの暗さを保つ。`--accent-text`は
  「本文の背景に直接乗る文字・細い枠線」(見出し・リンク・ホバー色)用。ダークテーマでは
  この2つを同じ値にできない(白文字が読めるくらい暗い色と、暗い背景の上で読めるくらい
  明るい色を同時には満たせない)。どちらに倒すかは「塗り+白文字」か「文字が背景に直接乗るか」
  で機械的に判定した(全16箇所の`var(--accent)`利用を1つずつ確認して分類)
- **ハイライト4色(黄緑青ピンク)と検索ヒットの配色はテーマ間で変えない**。その代わり
  `mark.search-hit`/`mark.hl`の文字色を`inherit`から固定の`--mark-fg`(常に濃色)に変えた。
  `inherit`のままだとダークテーマの明るい`--fg`を拾って、明るいパステル背景に明るい文字という
  読めない組み合わせになっていた
- **フォーム(`input[type="text"]`, `textarea`)にも`--surface`/`--fg`を明示**。素のブラウザ既定
  (白地に黒文字)のままだとダークテーマの時だけ周囲から浮いて見えるため
- **`.book-item-progress`等4箇所の`rgba(107, 79, 59, 0.0X)`を`--accent-tint`に統合**。
  アルファ値の微妙な違い(0.06〜0.1)は視覚的に無視できる差のため1つにまとめた
- **`#update-toast-reload`と`.hl-swatch.selected`は意図的に`--accent`のまま**(`--accent-text`に
  しない)。前者は固定の白背景チップに乗る文字、後者はパステルのスウォッチに乗る枠線で、
  どちらも「白系の上に置いても読める濃さ」が必要なため
- **設定はフォントサイズと同じくlocalStorageのみ**(`mdViewer.theme`)。バックアップJSON・
  Drive同期の対象外(ピン留め・最近読んだと同じ、端末ローカルの表示設定という扱い)
- **ボトムバーに「表示」ボタンを追加**(8個目)。しおり・ハイライトと同じシートの仕組みで
  テーマの3択(セピア/ライト/ダーク)を出す。本棚側は既存の「バックアップ」パネル内に
  同じ3択を追加し、両方が同じ`localStorage`を見て同期する(`.theme-picker button`という
  共通クラスで両方のボタン群を一括して`.active`更新している)
- **8ボタンになったことで375px幅(iPhone SE)で「ハイライト」が見切れた**ため、
  420px以下のブレークポイントに加えて390px以下でさらにフォントを縮める段を追加した
  (0.7rem→0.62rem)。ボタンを1個増やすたびにこの実機幅での見切れを必ず確認すること
- `<meta name="theme-color">`をテーマ切り替え時にJSで書き換える(OSのステータスバー・
  タスク切替カードの色をヘッダーの実際の色に合わせる)。`manifest.json`側の`theme_color`は
  インストール時のスプラッシュ画面にのみ影響するため対象外(実行中は変更不可)

## 目次に現在地を表示(2026-08-23)

62万字級の原稿だと「今どの章にいるか」が本文だけでは分からなくなるため、
目次を開いた時に今読んでいる章をハイライト+自動スクロールするようにした。

- **見出しの文字オフセットを`buildToc()`時に記録**する(`getElementOffsetInViewer()`。
  `Range.setEnd(見出し要素, 0)`で「見出しの直前まで」の文字数を測る、既存の
  `getTextOffsetInViewer()`と同じ手法)。既読位置・しおりと同じ「文字オフセット」基準に
  揃えたので、既存の`getOffsetAtViewportTop()`とそのまま比較できる
- **「現在地」は目次を開いた瞬間に計算するだけ**(開いている間ずっと追従はしない)。
  検索パネルなどと同じく、シートは開いた時の状態を見せるという既存の設計に合わせた
- **見出し出現順=オフセット昇順を利用**して、現在位置以下で最後の見出しを「今いる章」とする
  (`tocEntries`を先頭から見て、オフセットを超えたら直前の要素で確定)
- **番号だけの見出し(「症例史 › III.」等)にもそのままハイライトが乗る**。
  文脈補完はテキスト表示だけの話で、オフセット自体は見出し要素ごとに独立して持っているため
- `.current`のスタイルは`box-shadow: inset`を使った(`border-left`にすると、見出しレベルごとに
  JSでインライン指定している`padding-left`と衝突してズレるため)

## 検索結果一覧表示(2026-08-23)

「↑↓で1件ずつ移動」だけだったファイル内検索に、前後の文脈付きヒット一覧を追加した。

- **文脈は`<mark>`のDOM挿入後にpreviousSibling/nextSiblingのテキストノードから直接切り出す**
  (`renderSearchResultsList()`)。`highlightMatches()`の正規表現ループ側にスニペット保持を
  持ち込まず、後段で軽く1パスするだけにした。マーク同士が隣接していて兄弟がテキストノードでない
  (例:同じ語の連続ヒットで間に文字が無い)場合は、その側の文脈が空になるだけで壊れない
- **一覧タップでのジャンプは`closeAllSheets()`を通さない**。`closeAllSheets()`は
  `isSheetOpen('search')`なら必ず`closeSearch()`を呼び、検索語・ハイライト・一覧を
  全部消してしまう(バックドロップタップ・Escapeでの「検索を終える」時の意図通りの動作)。
  一覧タップは「他の候補と見比べたい」が主目的なので、見た目だけシートを閉じる
  `hideSheetVisualOnly('search')`を新設して使う。これにより検索ボタンを再度押すと、
  同じ検索語・同じ一覧・同じハイライト位置がそのまま復元される
  (`searchMatches`等の状態を一切消していないため)
- **一覧の`.active`更新は`updateActiveMatch()`に相乗り**させ、↑↓ボタン・一覧タップ・
  文中検索ジャンプ(選択テキストからの遷移)のどれでも一覧側のハイライトと自動スクロール
  (`block:'nearest'`。本文側の`block:'center'`とは別)が追従する。`match.li`に生成した
  `<li>`を保持しておき、↑↓のたびに500件を再構築せずに済むようにしている
- 1件あたり2行までにクランプ(`-webkit-line-clamp`)。500件上限に達していても
  一覧はそのまま500件分描画する(軽いDOM構造なので実用上問題にならないことを確認済み)
